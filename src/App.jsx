...
// This `appId` is provided by the environment when deployed.
// We fall back to your project's `appId` for local development.
const appId = typeof __app_id !== 'undefined' ? __app_id : '1:427094872698:web:873d739b58971fa62a61c3';
...
const BubbleBackground = ({ theme }) => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            if (Math.random() > 0.9) createBubble();
...
const useSimulatedPrices = (symbols, onPriceTick) => {
    ...
    useEffect(() => { onPriceTickRef.current = onPriceTick; }, [onPriceTick]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            symbolsToUpdate.forEach(symbolName => {
                ...
                const move = (Math.random() - 0.5) * (symbolInfo.pips * 10);
                ...
                if (onPriceTickRef.current) onPriceTickRef.current(symbolName, newBid, newAsk);
            });
...
const DemoTrader = ({ onTradeClose, theme, availableCapital, onSymbolVisibilityChange, hiddenSymbols, onNewMessage }) => {
    ...
    const checkPriceTriggers = useCallback((symbolName, bid, ask) => {
        setPositions(currentPositions => {
            ...
            const updatedPositions = currentPositions.filter(pos => {
                if (pos.symbol !== symbolName) return true;
    ...
    useEffect(() => {
        if (window.TradingView && tvWidgetRef.current && (mobileTab === 'chart' || window.innerWidth >= 768)) {
            ...
            // NEW: Redraw orders when chart reloads
            widget.onChartReady(() => {
                const currentChart = tvWidgetInstance.current.chart();
                const chartOrders = [];
                positions.forEach(pos => {
                    if (pos.symbol === selectedSymbol.name) {
                        const order = drawOrderLine(pos);
                        chartOrders.push(order);
                    }
                });
                setOpenOrders(chartOrders);
            });
        }
    }, [selectedSymbol, theme, mobileTab]); // Re-render if mobileTab changes to chart
    ...
    const handleMarketOrder = (type) => {
        ...
        if (marginRequired > freeMargin) { showMessage("Not enough free margin", false); return; }
        ...
        // NEW: Draw order on chart
        const chartOrder = drawOrderLine(newPosition);
        newPosition.chartOrder = chartOrder; // Store reference to chart line
        ...
        <div className="w-full p-4 flex flex-col space-y-3">
            ...
            <div className="flex-1 flex flex-col justify-between items-center">
                ...
                <div className="w-full flex gap-2 mt-4">
                    <button onClick={() => handleMarketOrder('SELL')} disabled={!currentSymbolPrices} className="flex-1 bg-red-600/90 hover:bg-red-500/90 border border-red-400 text-white p-3 rounded-md disabled:opacity-50">
                        <span className="font-bold text-lg">SELL</span>
                        <div className="font-mono text-xs">{currentSymbolPrices?.bid.toFixed(5)}</div>
                    </button>
                    <button onClick={() => handleMarketOrder('BUY')} disabled={!currentSymbolPrices} className="flex-1 bg-green-600/90 hover:bg-green-500/90 border border-green-400 text-white p-3 rounded-md disabled:opacity-50">
                        <span className="font-bold text-lg">BUY</span>
                        <div className="font-mono text-xs">{currentSymbolPrices?.ask.toFixed(5)}</div>
                    </button>
                </div>
            </div>
        </div>
...
const AuthComponent = ({ onAuthSuccess }) => {
    ...
    const handleEmailPass = async (e) => {
        ...
        try {
            ...
            // onAuthStateChanged will handle the auth success
        } catch (err) {
...
// --- Main Journal/Trader App Component ---
const JournalAppComponent = ({ user, userData, onSignOut }) => {
    // --- STATE ---
    const [data, setData] = useState(userData); // Use userData from parent
    ...
    // Update local state if userData from parent changes
    useEffect(() => {
        setData(userData);
    }, [userData]);
    ...
    const handleMouseDown = useCallback((e) => {
        ...
        if (e.button !== 0 || tagsToIgnore.includes(e.target.tagName) || e.target.closest('label') || e.target.classList.contains(classToIgnore) || e.target.closest(`.${classToIgnore}`)) return;
            ...
            <div className="w-full max-w-7xl perspective-container z-10">
                <div className="space-y-6">
                    {currentMonthData && currentMonthData.days.map((row, i) => (
                        <JournalDay key={row.date} row={row} activeMonthIndex={activeMonthIndex} i={i} todayString={todayString} handleMouseDown={handleMouseDown} handleDetailsChange={handleJournalChange} handleRuleChange={handleRuleChange} handleProfitInput={handleProfitInput} handleSignChange={handleSignChange} />
                    ))}
                </div>
            </div>
...
export default function MainApp() {
    ...
    useEffect(() => {
        if (user) {
            setDbLoading(true);
            // This path MUST match your security rules
